defmodule SocialScribe.SalesforceApiPropertyTest do
  ...
  describe "apply_updates/3 properties" do
    ...
    property "returns {:ok, :no_updates} when all updates have apply: false", %{credential: credential} do
      check all updates <- list_of(update_generator(apply: false), min_length: 1, max_length: 10) do
        result = SocialScribe.SalesforceApi.apply_updates(credential, "003xx000001234", updates)
  ...
  describe "Salesforce field mapping" do
    property "Salesforce contact fields are valid API field names" do
      check all field <- member_of(@salesforce_fields) do
        # Salesforce API field names should be lowercase and match their schema
        assert is_binary(field)
        assert String.length(field) > 0
        # Salesforce standard field names are lowercase
        assert field == String.downcase(field)
      end
    end
  end
  ...
  defp salesforce_contact_id_generator do
    # Salesforce Contact IDs start with "003" and are 18 characters
